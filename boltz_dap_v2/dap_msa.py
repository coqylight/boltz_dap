...
def _pwa_s_chunk_size(num_s: int) -> int:
    ...
    # Default caps peak for o_chunks @ W.T on large S (e.g. pentadecamer + 16 GPU).
    return min(num_s, 32)
...
def _pwa_proj_o_matmul_s_chunked(o_chunks: Tensor, proj_o_weight_T: Tensor) -> Tensor:
    ...
    b_b, _, n_tok, _ = o_chunks.shape
    out_d = proj_o_weight_T.shape[1]
    out = torch.empty(
        b_b, s_dim, n_tok, out_d,
        dtype=o_chunks.dtype,
        device=o_chunks.device,
    )
    for s0 in range(0, s_dim, cs):
        s1 = min(s0 + cs, s_dim)
        out[:, s0:s1].copy_(
            o_chunks[:, s0:s1].contiguous() @ proj_o_weight_T
        )
    return out
